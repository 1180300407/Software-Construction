...
public class TrainBoard implements Iterable<TrainEntry<Carriage>>{
	...
	private static Logger myLogger=Logger.getLogger("TrainBoardLog");
	...
	private void checkRep() {
		...
		FileHandler handler;
		try {
			handler = new FileHandler("src/LogFile/TrainBoardLog.log");
	...
	public void sortTrains() {
		...
		TreeMap<Long, TrainEntry<Carriage>> treeMap1=new TreeMap<>(new MapvalueComparator());//用treemap排序
		TreeMap<Long, TrainEntry<Carriage>> totaltreeMap=new TreeMap<>(new MapvalueComparator());//用treemap排序整体
		for(Map.Entry<TrainEntry<Carriage>, Long> entry:reachTrains.entrySet()) {//排序抵达车次
			treeMap1.put(entry.getValue(), entry.getKey());
			totaltreeMap.put(entry.getValue(), entry.getKey());//总体排序
		}
		TreeMap<Long, TrainEntry<Carriage>> treeMap2=new TreeMap<>(new MapvalueComparator());
		for(Map.Entry<TrainEntry<Carriage>, Long> entry:leaveTrains.entrySet()) {//排序离开车次
			treeMap2.put(entry.getValue(), entry.getKey());
			totaltreeMap.put(entry.getValue(), entry.getKey());
		}
		for(Map.Entry<Long,TrainEntry<Carriage>> entry:treeMap1.entrySet()) {
			sortreach.put(entry.getValue(), entry.getKey());//排序后的map
		}
		for(Map.Entry<Long,TrainEntry<Carriage>> entry:treeMap2.entrySet()) {
			sortleave.put(entry.getValue(), entry.getKey());//排序后的map
		}
		
		this.reachTrains=sortreach;
		this.leaveTrains=sortleave;//现在是有序的
		trains=new ArrayList<TrainEntry<Carriage>>();//将整体排序的treemap赋给trains
		
		for(Map.Entry<Long,TrainEntry<Carriage>> entry:totaltreeMap.entrySet()) {
			trains.add(entry.getValue());
		}
	...
	class MapvalueComparator implements Comparator<Long>{
