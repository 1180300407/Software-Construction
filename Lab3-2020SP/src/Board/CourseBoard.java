...
public class CourseBoard implements Iterable<CourseEntry<Teacher>>{
	...
	public void visualize() throws ParseException {
        ...
        for(int i=0;i<courses.size();i++) {
			...
			String teachers=cpe.getResource().get(0).getName();
			for(int j=1;j<cpe.getResource().size();j++) {//多名教师
				teachers=teachers.concat(","+cpe.getResource().get(j).getName());
			}
			rowData[i][2]=teachers;
	...
	public static void main(String[] args) throws ParseException {
		...
		Timeslot timeslot=new Timeslot("2020-05-11 16:20", "2020-05-11 19:20");//根据现实时间调整测试时间,日期应设置为与测试日期相同
		...
		Timeslot timeslot2=new Timeslot("2020-05-11 18:30", "2020-05-11 19:50");//根据现实时间调整测试时间,日期应设置为与测试日期相同
